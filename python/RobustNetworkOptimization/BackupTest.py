...
#        if n[i,j] > 0:
    ...
    start = time.clock()
    BufferedP = GetBufferedFailureProbPar(scenarios, num_scenarios[3], links, BkpLinks, OptCapacity, BackupLinks)
    stop = time.clock()        
    print('[%g seconds]Done!\n'%(stop-start))
...
def GetBufferedFailureProbPar(Scenarios, NumScenarios, Links, BackupLinks, CapPerBackupLink, OptBackupLinks):    
#     info('function GetRandScenariosPar')
    ...
    Sum=0
    for k in range(nb_processes):
        ScenarioIndex[k]=k*Dividend
        Size[k]=Dividend
        Sum=Sum+Size[k]
        if k == nb_processes-1:
            Size[k]=Size[k]+NumScenarios-Sum
        args[k]=(Scenarios,ScenarioIndex[k], Size[k], Links, BackupLinks, CapPerBackupLink, OptBackupLinks)
...
def ThreadGetBufferedFailureProb(Scenarios, Index, NumScenarios, Links, BackupLinks, CapPerBackupLink, OptBackupLinks):    
    ...
    for i,j in BackupLinks:
        ...
        for k in range(NumScenarios):
            Psd=0
            for s,d in Links:
                Psd=Psd+OptBackupLinks[i,j,s,d]*Scenarios[Index+k,s,d]
            if Psd > CapPerBackupLink[i,j]:
                P[i,j]=P[i,j]+1
        P[i,j]=1.0*P[i,j]/NumScenarios
    return P